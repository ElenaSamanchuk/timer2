 const endDate = new Date("2025-07-10T23:59:59");
    ...
    function updateTimer() {
      ...
      if (diff <= 0) {
        container.innerHTML = "<div class='segment'><div class='digit'>00</div><div class='label'>завершено</div></div>";
        return;
      }
      ...
      const segments = [
        ...
        { val: minutes, label: ["минута", "минуты", "минут"] },
        {
          ...
          isLastDigit: true,
          content: `
            <span>${secondLeft}</span><span class="last-digit-inner${lastChanged ? ' animate' : ''}">${secondRight}</span>
          `
        },
      ];
      ...
      container.innerHTML = segments.map((seg, idx) => {
        ...
        return `
          <div class="segment">
            <div class="digit ${isLast ? 'last-digit' : ''}">${isLast ? seg.content : [...val].map(d => `<span>${d}</span>`).join('')}</div>
            <div class="label">${getWordForm(isLast ? Number(secondLeft + secondRight) : seg.val, seg.label)}</div>
          </div>
        `;
      }).join("");
